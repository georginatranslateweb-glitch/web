...
const ServiceSection = ({ headingTitle, title, children, flush = false }) => {
    ...
    return (
        <motion.div
            className={`team-rules-area service-accordion${open ? ' is-open' : ''}${flush ? ' service-accordion--flush' : ''}`}
            variants={fadeUp}
            initial="hidden"
            whileInView="visible"
            viewport={VIEWPORT_ONCE}
        >
            {flush ? inner : <div className="container">{inner}</div>}
        </motion.div>
    );
};
